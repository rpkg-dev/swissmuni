...
-   The returned names seem to be right (there are columns with completely different IDs like `HR_ARBREG2000_L2` and `HR_TYPMSR2000_L2` but identical values), except some year numbers. They are corrected by the function below.

-   The `MS` in the full column names returned by `gen_col_spec_classifications()` stands for *mobilité spatiale*, cf. <https://www.bfs.admin.ch/bfs/de/home/statistiken/raum-umwelt/nomenklaturen/msreg.assetdetail.415729.html>
...
|-------------------------------------------|--------------------------------------------|
| CODE\_HIST\_1\_Text\_en                   | CODE\_OFS\_1\_Text\_en                     |
| CODE\_HIST\_1\_Text\_fr                   | CODE\_OFS\_1\_Text\_fr                     |
| CODE\_HIST\_1\_Text\_de                   | CODE\_OFS\_1\_Text\_de                     |
| CODE\_HIST\_1\_Text\_it                   | CODE\_OFS\_1\_Text\_it                     |
| CODE\_HIST\_1\_Text                       | CODE\_OFS\_1\_Text                         |

The `CODE_OFS*` corresponds to the ["GEOSTAT"/"BFS" number](https://de.wikipedia.org/wiki/Gemeindenummer) and the `CODE_HIST*` to the "historical number". -\> The URL parameter's name `useBfsCode` seems misleading (inversed)!
...
-   Setting `format` to anything else than `csv` results in a **XLS response**!
    ...
    -   For the `levels` endpoint, the XLS contains the **column name spec** of the normal API response's `HR_*` columns. The `levels` endpoint is also the only one offering the `labelLanguages` URL param (see below).
...
This function parses the API's [Web Services Description Language (WSDL)](https://en.wikipedia.org/wiki/Web_Services_Description_Language) file to determine possible URL parameters.
...
-   The API returns different numbers of columns depending on the provided URL params (like the `start/end_date`). Therefore we generate the column type spec telling `readr::read_csv()` how to parse the results dynamically.
