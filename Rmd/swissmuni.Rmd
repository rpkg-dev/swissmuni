...
- The return value of `get_classifications_col_spec()` has been written to the package file `inst/extdata/col_spec_classifications.Rds` using
  `save_rds(get_classifications_col_spec(), path = "inst/extdata/col_spec_classifications.Rds")`.
...
## API response's column names

- To determine them for the `snapshots` endpoint, use:
...
- The `MS` in the full column names returned by `get_classifications_col_spec()` stands for _mobilité spatiale_, cf.
